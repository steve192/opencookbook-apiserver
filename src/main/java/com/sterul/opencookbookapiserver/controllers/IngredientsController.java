...
public class IngredientsController extends BaseController {
    ...
    public List<IngredientResponse> all() {
        return ingredientService.getUserPermittedIngredients(getLoggedInUser()).stream().map(this::entityToResponse)
                .toList();
    }
    ...
    private IngredientResponse entityToResponse(Ingredient ingredient) {
        var nutrientInfo = ingredient.getIngredientNutritionalInfo();
        var ingredientBuilder = IngredientResponse.builder()
                .id(ingredient.getId())
                .name(ingredient.getName());

        if (nutrientInfo != null) {
            ingredientBuilder
                    .ingredientNutritionalInfo(IngredientNutrientInfoResponse.builder().id(nutrientInfo.getId())
                            .energyPerUnit(nutrientInfo.getEnergyPerUnit()).build());
        }
